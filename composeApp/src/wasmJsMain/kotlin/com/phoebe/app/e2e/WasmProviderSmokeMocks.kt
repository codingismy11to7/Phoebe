...
internal fun plexCatalogMockEngine(): MockEngine = MockEngine { request ->
    when (request.url.encodedPath) {
        "/library/sections/1/all" -> if (request.url.parameters["type"] == "10") {
            respondJson(wasmAlbumTracksJson())
        } else {
            respondJson(wasmArtistsJson())
        }
        "/library/metadata/a1/children" -> respondJson(wasmAlbumTracksJson())
        "/playlists" -> when (request.method.value) {
            "POST" -> respondJson(wasmCreatedPlaylistJson())
            else -> respondJson(wasmPlaylistsJson())
        }
        "/playlists/p1/items" -> when (request.method.value) {
            "PUT" -> respondJson("""{ "MediaContainer": { "leafCountAdded": 1, "Metadata": [] } }""")
            else -> respondJson("""{ "MediaContainer": { "Metadata": [] } }""")
        }
        "/identity" -> respondJson("""{ "MediaContainer": { "machineIdentifier": "server" } }""")
        else -> respond("", HttpStatusCode.NotFound)
    }
}

private fun wasmArtistsJson(): String = """
    { "MediaContainer": { "Metadata": [ { "ratingKey": "artist1", "type": "artist", "title": "Artist One" } ] } }
""".trimIndent()

private fun wasmAlbumTracksJson(): String = """
    {
      "MediaContainer": {
        "Metadata": [
          {
            ...
            "duration": 1000,
            "Media": [ { "Part": [ { "key": "/library/parts/t1/file.mp3", "file": "file.mp3" } ] } ]
          }
        ]
      }
    }
...
private fun wasmPlaylistsJson(): String = """
    { "MediaContainer": { "Metadata": [ { "ratingKey": "p1", "title": "Playlist One", "leafCount": 2, "key": "/playlists/p1/items" } ] } }
""".trimIndent()

private fun wasmCreatedPlaylistJson(): String = """
    { "MediaContainer": { "Metadata": [ { "ratingKey": "p99", "title": "Smoke Mix", "leafCount": 1, "key": "/playlists/p99/items" } ] } }
""".trimIndent()
