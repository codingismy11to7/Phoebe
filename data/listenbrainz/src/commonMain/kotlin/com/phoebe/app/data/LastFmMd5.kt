...
private val LastFmMd5Shifts = intArrayOf(
...
private val LastFmMd5Table = IntArray(64) { i ->
    (kotlin.math.abs(kotlin.math.sin((i + 1).toDouble())) * 4294967296.0).toLong().toInt()
}

internal fun md5HexForLastFm(input: String): String {
    ...
    for (offset in padded.indices step 64) {
        ...
        for (i in 0 until 64) {
            ...
            d = c
            c = b
            b += (a + f + LastFmMd5Table[i] + m[g]).rotateLeft(LastFmMd5Shifts[i])
            a = temp
        }
